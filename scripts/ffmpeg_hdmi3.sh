...
SCALE_FLAGS="${SCALE_FLAGS:-bicubic}"
...
exec ffmpeg \
  ...
  -f alsa -i "$AUDIO_DEV" \
  -fflags +genpts \
  -filter_complex "[0:v]split=3[v480src][v360src][v180src];[v480src]scale=854:480:flags=${SCALE_FLAGS},fps=${OUTPUT_480_FPS}[v480];[v360src]scale=640:360:flags=${SCALE_FLAGS},fps=${OUTPUT_360_FPS}[v360];[v180src]scale=320:180:flags=${SCALE_FLAGS},fps=${OUTPUT_180_FPS}[v180];[1:a]aresample=async=1000:min_hard_comp=0.100:first_pts=0,asplit=3[a480][a360][a180]" \
  -map "[v480]" -map "[a480]" \
  -map "[v360]" -map "[a360]" \
  -map "[v180]" -map "[a180]" \
  -c:v libx264 -preset "$X264_PRESET" -tune zerolatency \
  -pix_fmt yuv420p \
  -b:v:0 1050k -maxrate:v:0 1200k -bufsize:v:0 2400k \
  -b:v:1 760k -maxrate:v:1 900k -bufsize:v:1 1800k \
