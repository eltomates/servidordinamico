...
# Segundo dongle USB (puerto pci-0000:00:14.0 usb-0:10)
...
LOCK_FILE="$LOCK_DIR/hdmi2_ffmpeg.lock"
mkdir -p "$OUT/480p" "$OUT/360p" "$OUT/180p" "$LOCK_DIR"
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "[ffmpeg_hdmi2] ya hay otra instancia activa; saliendo." >&2
  exit 0
fi

exec ffmpeg \
  -thread_queue_size 2048 \
  -probesize 10000000 -analyzeduration 2000000 \
  -f v4l2 -input_format "$INPUT_FORMAT" -framerate "$INPUT_FPS" -video_size 1280x720 -i "$VIDEO_DEV" \
