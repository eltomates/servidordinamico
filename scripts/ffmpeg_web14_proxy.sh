...
# Entrada:  https://pluto.tv/latam/live-tv/5ddd7cb2cbb9010009b4fe32
# Salida:   /var/www/html/hls/web14/index.m3u8 (accesible como /hls/web14/index.m3u8)
...
LAST_RESOLVED_FILE="${LAST_RESOLVED_FILE:-/var/www/html/logs/web14_last_resolved.url}"
...
resolve_pluto_source_url() {
  ...
  if [ "$PLUTO_REQUIRE_CHROMIUM" = "1" ]; then
    echo "[ffmpeg_web14_proxy] Chromium no resolvio URL valida para Pluto; reintentando sin fallback directo." >&2
    return 1
  fi
...
if not match:
    raise SystemExit(f"URL de Pluto no válida: {page_url}")
...
resolve_source_url_with_timeout() {
  ...
  tmp_file="$(mktemp /tmp/web14_resolve.XXXXXX)" || return 1
  ...
  while kill -0 "$resolve_pid" 2>/dev/null; do
    if [ "$waited" -ge "$RESOLVE_TIMEOUT_SECONDS" ]; then
      echo "[ffmpeg_web14_proxy] Resolucion de origen tardo mas de ${RESOLVE_TIMEOUT_SECONDS}s; cancelando intento." >&2
...
run_ffmpeg_from_direct_url() {
  ...
  echo "[ffmpeg_web14_proxy] Usando origen directo: $input_url" >&2
...
run_ffmpeg_from_ytdlp() {
  ...
  if ! command -v yt-dlp >/dev/null 2>&1; then
    echo "[ffmpeg_web14_proxy] yt-dlp no está instalado; no se puede resolver $source_url" >&2
    return 1
  fi

  echo "[ffmpeg_web14_proxy] Resolviendo y enviando por tubería: $source_url" >&2
...
while true; do
  ...
  START_NUMBER="$(date +%s)"
  if [ "$USE_LAST_RESOLVED_ON_START" = "1" ] && [ "$LAST_RESOLVED_BOOT_USED" -eq 0 ] && [ -s "$LAST_RESOLVED_FILE" ]; then
    RESOLVED_SRC_URL="$(cat "$LAST_RESOLVED_FILE")"
    echo "[ffmpeg_web14_proxy] Usando ultima URL HLS buena para arrancar rapido." >&2
  ...
  if { [ "$resolve_rc" -ne 0 ] || [ -z "$RESOLVED_SRC_URL" ]; } && [ -s "$LAST_RESOLVED_FILE" ]; then
    RESOLVED_SRC_URL="$(cat "$LAST_RESOLVED_FILE")"
    echo "[ffmpeg_web14_proxy] Usando ultima URL HLS buena tras fallo de resolucion." >&2
    resolve_rc=0
  fi
  ...
  while kill -0 "$ffmpeg_pid" 2>/dev/null; do
    ...
    if [ -f "$OUT/index.m3u8" ] && grep -q '^#EXT-X-ENDLIST' "$OUT/index.m3u8" 2>/dev/null; then
      echo "[ffmpeg_web14_proxy] index.m3u8 contiene #EXT-X-ENDLIST, reiniciando ffmpeg ($ffmpeg_pid)" >&2
